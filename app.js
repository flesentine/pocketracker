...
function isPocketTrackerIOS() {
  return Boolean(window.webkit?.messageHandlers?.pocketTracker);
}

function askText({ title, value = "", placeholder = "", confirmLabel = "Done" }) {
  return new Promise((resolve) => {
    const overlay = document.createElement("div");
    overlay.className = "text-dialog";
    overlay.innerHTML = `
      <form class="text-dialog-panel">
        <label>
          <span></span>
    ...
    const titleEl = overlay.querySelector("span");
...
async function downloadBlob(blob, filename) {
  if (isPocketTrackerIOS()) {
    window.webkit.messageHandlers.pocketTracker.postMessage({
      action: "shareFile",
      filename,
...
async function renameActivePattern() {
  const currentName = patterns[activePatternIndex]?.name || "";
  const requestedName = await askText({
    title: "Name this pattern",
    value: currentName || getDefaultPatternName(activePatternIndex),
    confirmLabel: "Rename",
  });
  if (requestedName === null) return;
